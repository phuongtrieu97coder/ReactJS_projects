import React,{useState,useEffect,Suspense,startTransition,PureComponent} from "react";
...
//you can add the block of syntax below instead to make the effective for your slideshow
             if(scrollposition ===0){
                 setScrollposition(0);
             }else{
               var num = 0;
               var increase1 =  setInterval(()=>{
                   num+=1;
                 setScrollposition(c=>c-num);
                 if(num === 25){
...
//you can add the block of syntax below instead to make the effective for your slideshow
        ...
        if(scrollposition === 2925){
            setScrollposition(2925);
        }else{
            var num = 0;
            var increase1 =  setInterval(()=>{
                num+=1;
      ...
      <ErrorBoundaries>
          <Suspense fallback={<h1>Loading...</h1>}>
              <div id="Contentblockcode1">
                    <List className="Table1" itemCount={data1block.length} itemSize={400}
                    width={1150} height={330}
                    layout="horizontal">
                        {Cell}
                    </List>
{/* 
